...
displayProduct(product_arr)
const categoryFilter=document.getElementById('filterCatergory')
...
function applyfilter(){
    ...
    const filterProducts=product_arr.filter(product_arr=>!category || product_arr.category==category);
...
function applysort(){
    ...
    const sortedProducts=[...product_arr]
...
function displayProduct(product_arr){
...
function createProductCard(product){
    ...
    const strickprice=document.createElement('p')
    //strickprice.textContent=product.strikePrice
    strickprice.textContent = `strikePrice: $${product.strikePrice.toFixed(2)}`;

    const categoryss=document.createElement('p')
    categoryss.textContent=product.category;
    
    

    const quantity=document.createElement('b')
    //quantity.textContent=product.quantity
    quantity.textContent = `quantity: ${product.quantity.toFixed(2)}`;
    ...
    card.appendChild(strickprice)
    card.appendChild(button)
    card.appendChild(categoryss)
